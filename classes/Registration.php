...
class Registration
{
    ...
    public function __construct()
    {
        ...
        if (!empty($_POST["name"])) {
            $this->errors[] = "Potential bot request";
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat)
    {
        ...
        $this->createUserDir($user_name);

        $db = db_connect("kolour");
        $c = $db->profile;
        $c->ensureIndex(array('user_email' => 1), array('unique' => true));
    ...
    {
        $user_dir = $_SERVER['DOCUMENT_ROOT'] . '/users/';
        if (@mkdir($user_dir . $username, 0755, true)) {
            // echo "Directory for " . $username . " successfully created.";
            return true;
        } else {
            if (rmdir($user_dir . $username)) {
                mkdir($user_dir . $username, 0755, true);
                // echo "Directory has been overwritted";
            }
        }
    }
}
